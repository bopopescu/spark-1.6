...
private[netty] class NettyRpcEnv(
  ...
  // TransportContext用来创建server和clientFactory
  ...
  /**
    ...
    * 此外还有用于处理信息的MessageEncoder、MessageDecoder、RpcHandler等，这些和netty的具体使用紧密相关
    * 关于Spark Network Common的实现 https://github.com/ColZer/DigAndBuried/blob/master/spark/spark-network-netty.md
    * @param port
    */
  def startServer(port: Int): Unit = {
    ...
    // 启动netty server
    // TransportContext用来创建netty server和clientFactory
...
private[netty] class NettyRpcEnvFactory extends RpcEnvFactory with Logging {
  ...
  def create(config: RpcEnvConfig): RpcEnv = {
    ...
    // 序列化,现使用JavaSerializer,未来会引入KryoSerializer,看上面的注释
    // Kryo是一个快速高效的Java对象序列化框架，旨在提供快速、高效和易用的API。
    // 无论文件、数据库或网络数据Kryo都可以随时完成序列化。Kryo还可以执行自动深拷贝、浅拷贝。
    val javaSerializerInstance =
      new JavaSerializer(sparkConf).newInstance().asInstanceOf[JavaSerializerInstance]
    val nettyEnv =
      new NettyRpcEnv(sparkConf, javaSerializerInstance, config.host, config.securityManager)

    // 非client, 启动netty server,提供底层RPC服务
    if (!config.clientMode) {
      // 定义启动netty的方法startNettyRpcEnv，输入为端口号
      // startNettyRpcEnv为函数,作为参数传入到Utils.startServiceOnPort中启动server
      // 其实最根本还是startNettyRpcEnv函数内部nettyEnv.startServer方法
      val startNettyRpcEnv: Int => (NettyRpcEnv, Int) = { actualPort =>
        // 启动netty server的最根本方法
        nettyEnv.startServer(actualPort)
        (nettyEnv, nettyEnv.address.port)
      }
      try {
        // 使用Utils.startServiceOnPort启动server
        Utils.startServiceOnPort(config.port, /*函数参数*/ startNettyRpcEnv,
          sparkConf,
          config.name)._1
      } catch {
        case NonFatal(e) =>
