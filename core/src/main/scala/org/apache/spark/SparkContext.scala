 ...
 *   this config overrides the default configs as well as system properties.
  *
  *   Spark程序的入口,一个SparkContext代表了一个application
...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  try {
    ...
    //判断spark.master和spark.app.name是否存在
    ...
    // 如果是yarn cluser模式则必须设置spark.yarn.app.id.
    // 这里涉及spark on yarn问题,spark-submit向yarn提交后,yarn分配的spark.yarn.app.id
    ...
    // _eventLogDir是否记录运行时信息，由spark.eventLog.enabled和spark.eventLog.dir控制
    _eventLogDir =
    ...
    // spark.externalBlockStore.folderName设置运行时本地存储的目录名，为“spark-” + randomUUID.toString()
    _conf.set("spark.externalBlockStore.folderName", externalBlockStoreFolderName)

    // 如果为yarn-client模式，设置SPARK_YARN_MODE=true，使用JobProgressListener
    // 跟踪运行时信息，用于UI展示
    if (master == "yarn-client") System.setProperty("SPARK_YARN_MODE", "true")

    // "_jobProgressListener" should be set up before creating SparkEnv because when creating
    // "SparkEnv", some messages will be posted to "listenerBus" and we should not miss them.
    // _jobProgressListener 在SaprkEnv之前设置,否则可能会丢失信息
    _jobProgressListener = new JobProgressListener(_conf)
    listenerBus.addListener(jobProgressListener)

    // Create the Spark execution environment (cache, map output tracker, etc)
    // 创建SparkEnv对象，创建SparkEnv的过程涉及到非常多spark-core中的核心类。
    _env = createSparkEnv(_conf, isLocal, listenerBus)
    SparkEnv.set(_env)

    // _metadataCleaner使用TimerTask定期清理persistentRdd
    ...
    // 将jar和file的路径添加到rpcEnv的fileServer，addJar和addFile内部调用rpcEnv
    if (jars != null) {
      jars.foreach(addJar)
    }
    ...
    // 根据资源管理机类型,创建对应的(SchedulerBackend,TaskScheduler),taskScheduler与schedulerBackend是一一对应的
    ...
    // DAGScheduler中包含了TaskScheduler的引用,必须在TaskScheduler创建之后再在创建
  ...
  def runJob[T, U: ClassTag](
    ...
    // 调用DAGScheduler.runJob(),生成task并提交
...
object SparkContext extends Logging {
  ...
  /**
    ...
    *
    * @param sc
    * @param master 根据传入的Master的URL的规则判断集群的部署方式(或者说是资源管理方式)，比如是Standalone,Mesos,YARN或者是Local等等
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      // val SPARK_REGEX = """spark://(.*)""".r   Standalone 模式
