...
class HadoopRDD[K, V](
  ...
  /**
    * spark调用hadoop api对HDFS中的数据进行切分,
    * 将hadoop中读取的inputSplits存入partition中
    * 一个hadoop的inputSplit 对应一个 spark的partition
    * http://www.cnblogs.com/xuepei/p/3664698.html
    * @return
    */
