...
/**
  * Master继承ThreadSafeRpcEndpoint，ThreadSafeRpcEndpoint继承RpcEndpoint。
...
private[deploy] class Master(
  ...
  override def onStart(): Unit = {
    ...
    // 启动定时任务，默认每60s send CheckForWorkerTimeOut信息给Master，检测Worker连接情况
    checkForWorkerTimeOutTask = forwardMessageThread.scheduleAtFixedRate(new Runnable {
      override def run(): Unit = Utils.tryLogNonFatalError {
        // 信息发送给 Master的receive方法，receive使用scala模式匹配处理信息
  ...
  override def receive: PartialFunction[Any, Unit] = {
    ...
    // 接受Worker发来的心跳Heartbeat
    case Heartbeat(workerId, worker) => {
      // 处理Worker发过来的信息，如果worker信息已经存在则更新lastHeartbeat
      // (在onStart方法中提到),否则重新连接master，也就是注册
    ...
    // 接受Master启动时,onStart中定时检查Woker是否超时
  ...
  // 同步方法,接收消息后要回应
  // receiveAndReply方法接收EndpointRef ask及其衍生方法发送的信息
    ...
    case RegisterWorker(
      ...
      if (state == RecoveryState.STANDBY) {
        ...
        if (registerWorker(worker)) {
          ...
          // registerWorker(worker)返回true, 也就是Worker信息注册成功,
          // 回复给Worker: RegisteredWorker(self, masterWebUiUrl)
  ...
  // 移除dead和unknown 的Worker, 然后注册传进来的Worker
  ...
  private def timeOutDeadWorkers() {
    ...
    // Master根据条件lastHeartbeat < currentTime - WORKER_TIMEOUT_MS
    // 判断Worker是否还在发送心跳，如果过期将其从对应集合中删除。
    // lastHeartbeat是Worker最后一次连接的时间(Worker 最后一次发送心跳的时间)，
...
private[deploy] object Master extends Logging {
  ...
  def main(argStrings: Array[String]) {
    ...
    // startRpcEnvAndEndpoint方法，初始化RpcEnv，注册Master(RpcEndpoint)到RpcEnv，
    // 到这里，上面介绍的生命周期onStart -> receive(receiveAndReply)* -> onStop方法开始工作
   ...
   *   (3) The REST server bound port, if any
    *   启动 Master 返回一个元祖, 包含 Master的RpcEnv,WebUI端口, Master RESTful接口服务
   */
