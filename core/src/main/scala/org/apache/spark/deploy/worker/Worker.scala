...
// Worker也是RpcEndpoint的子类，所以接下来查看RpcEndpoint生命周期的四个方法:
...
private[deploy] class Worker(
      ...
      registerMasterThreadPool.submit(new Runnable {
        override def run(): Unit = {
          try {
            ...
            // 返回和Master通信的EndpointRef，masterEndpoint
            val masterEndpoint =
              rpcEnv.setupEndpointRef(Master.SYSTEM_NAME, masterAddress, Master.ENDPOINT_NAME)
            // 调用带参registerWithMaster方法
  ...
  private def registerWithMaster() {
    ...
    registrationRetryTimer match {
      case None =>
        ...
        // Worker信息注册完成后会将registered属性设置为true
        // 此定时任务会定时检查registered,如果为false,可能已断开连接,这自动注册Worker
        // 注册逻辑和tryRegisterAllMaster相同
  ...
  /**
    * Worker向Master提交注册信息
    * @param masterEndpoint 代理Master的对象
    */
  private def registerWithMaster(masterEndpoint: RpcEndpointRef): Unit = {
    //调用RpcEndponitRef的ask方法给Master发送RegisterWorker 这个caseclass,注册Worker信息
    masterEndpoint.ask[RegisterWorkerResponse](RegisterWorker(
      workerId, host, port, self, cores, memory, webUi.boundPort, publicAddress))
      .onComplete {
        // This is a very fast action so we can use "ThreadUtils.sameThread"
        // Endpoint.ask()是同步方法,发出请求等待回应onComplete
        // onComplete返回类型Success(value: T)或者Failure(val exception: Throwable)
        case Success(msg) =>
  ...
  private def handleRegisterResponse(msg: RegisterWorkerResponse): Unit = synchronized {
    msg match {
      case RegisteredWorker(masterRef, masterWebUiUrl) =>
        ...
        //将registered属性设置为true，修改用于和Master通信的masterRef及masterWebUiUrl
        registered = true
        changeMaster(masterRef, masterWebUiUrl)

        // 接受到Master发来的RegisteredWorker后,Worker启动一个定时任务发送心跳
        forwordMessageScheduler.scheduleAtFixedRate(new Runnable {
          override def run(): Unit = Utils.tryLogNonFatalError {
            // Worker给自己发SendHeartbeat,
            // 由Endpoint生命周期中的receive方法接收到SendHeartbeat
  ...
  override def receive: PartialFunction[Any, Unit] = synchronized {
    ...
    //接受handleRegisterResponse的定时任务,向Master发送心跳
    case SendHeartbeat =>
