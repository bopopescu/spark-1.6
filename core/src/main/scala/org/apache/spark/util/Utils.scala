...
private[spark] object Utils extends Logging {
  ...
  */
   ...
   * @return (service: T, port: Int)
    *
    *  startServiceOnPort方法调用startService启动server，
    *  而startService就是startNettyRpcEnv方法。
   */
  def startServiceOnPort[T](
      startPort: Int,
      startService: Int => (T, Int), // 函数参数startNettyRpcEnv,内部调用nettyEnv.startServer
      conf: SparkConf,
    ...
    for (offset <- 0 to maxRetries) {
      ...
      try {
        // 启动netty server, startService是函数参数startNettyRpcEnv,
        // 内部调用nettyEnv.startServer
